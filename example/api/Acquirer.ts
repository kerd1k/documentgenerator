...
export class Acquirer extends BaseEndpoint {
   ...
   *
   * @example
   *	POST http://localhost/api/acquirer/add
   */
  async add(req: HTTPRequestVO): Promise<TransferPacketVO<IAcquirer | IError>> {
    ...
    if (ErrorUtils.isError(acquirer)) 
      return { error: acquirer as IError };
   ...
   */
   async list(req: HTTPRequestVO): Promise<TransferPacketVO<IAcquirer[] | IError>> {
    ...
    if (ErrorUtils.isError(acquirers)) 
      return { error: acquirers as IError };
   ...
   */
   async get(req: HTTPRequestVO): Promise<TransferPacketVO<IAcquirer | IError>> {
    ...
    if (id.toString() === "0") 
      acquirer = await GD.REQ_ACQUIRER_GET_EMPTY_OBJECT.request({});
    else
      acquirer = await GD.REQ_ACQUIRER_GET_BY_ID.request({data: {id}});
